...
export const TeacherGrading = () => {
  ...
  useSocketEvent('grade:updated', ({ answerId }: { answerId: string }) => {
    qc.invalidateQueries({ queryKey: ['answers'], exact: false })
    qc.invalidateQueries({ queryKey: ['answer', answerId] })
  })

  useSocketEvent('annotation:added', ({ answerId }: { answerId: string }) => {
    qc.invalidateQueries({ queryKey: ['answers'], exact: false })
    qc.invalidateQueries({ queryKey: ['answer', answerId] })
  })
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="border border-rule">
          <WorksheetTabs worksheets={worksheets} activeId={activeWorksheetId}
            onSelect={id => { setActiveWorksheetId(id); setSelectedAnswerId(null) }} />
          <div className="p-4">
            {allAnswers.length === 0 ? (
              ...
              <div className="space-y-2">
                {allAnswers.map(ans => (
                  <button key={ans.id} onClick={() => setSelectedAnswerId(ans.id)}
                    className={`w-full text-left p-3 border transition-colors ${selectedAnswerId === ans.id ? 'border-ink bg-ink text-paper' : 'border-rule bg-paper hover:bg-cream'}`}>
                    <div className="flex items-center justify-between mb-1">
                      <span className={`text-sm font-medium ${selectedAnswerId === ans.id ? 'text-paper' : ''}`}>{ans.student.name}</span>
                      <AnswerBadge status={ans.status} />
                    </div>
                    <p className={`text-xs font-mono truncate ${selectedAnswerId === ans.id ? 'text-paper/70' : 'text-ink/50'}`}>
                      {(ans as any).question?.text}
                    </p>
                    {ans.grade && (
                      <div className="flex items-center gap-2 mt-1">
                        <span className={`text-xs font-mono ${selectedAnswerId === ans.id ? 'text-paper/70' : 'text-ink/50'}`}>
